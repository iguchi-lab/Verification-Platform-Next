...
[自立循環プロジェクト](https://www.jjj-design.org/)の全寒暖冷房委員会で開発している検証用プラットフォームの計算プログラムです。
...
│   ├── inputs            # 入力パラメータの定義、処理
│   │   ├── ac_setting.py # 空調（暖房、冷房）の入力項目
│   │   ├── common.py     # 地域区分、床面積、外皮性能等のその他の入力項目
│   │   └── main.py       # 【その他】プログラムのエントリーポイント。Colabから呼び出される。
│   ├── section4_2.py     # 【その他】巨大関数 calc_Q_UT_A() の検証用PF版。
...
3. jjjexperiment/inputs/di_container.py に、データを登録する
    ...
    @singleton # おまじない。必須。
    @provider  # おまじない。必須。
    def create_max_power_consumption(self) -> MaxPowerConsumption: # 関数名は任意。戻り値の型は必須
        return MaxPowerConsumption.from_dict(self._input if self._input is not None else {})
    ```

4. データを取得する
   型を指定して引数に追加すると、関数呼び出し時に値がセットされます。

   main.py の calc_main() で取得する場合
    ```python
    def calc_main(
        ...
        max_power_consumption: MaxPowerConsumption # 型を指定すると、自動で与えられる
    ) -> dict | None:
    ```

   section4_2.py の calc_Q_UT_A() で取得する場合
    ```python
    @jjj_cloning
    @inject
    def calc_Q_UT_A(
