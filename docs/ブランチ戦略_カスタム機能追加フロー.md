# Git ブランチ戦略 カスタム機能追加フロー

## 概要

本リポジトリは建築研究所の `BRI-EES-House/pyhees` からフォークしており、独自機能を追加しながら開発を進めています。開発した独自機能はプラットフォームにて公開され、委員による検証を進めています。
...
```mermaid
sequenceDiagram
    participant Dev as 開発環境(ローカルPC等)
...
gitGraph
    ...
    commit id: "RE-CREATE develop" type: HIGHLIGHT
```
