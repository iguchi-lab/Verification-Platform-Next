...
2. 4つの`pyhees`ファイルに残る`pyhees -> jjjexperiment`逆依存を、依存元と検証方法ごとに分けて削減する。`section4_2_b.py`と`section4_3_a.py`の3スカラー依存はPR #130で除去済み。建研由来ファイルへの変更になるため、一般的な低リスクPRより小さくし、上流との差分追跡を優先する。
