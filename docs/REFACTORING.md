...
1. `section4_2_jjj.py`の`calc_Q_UT_A`と`main.py`の`calc_main`に残る長いオーケストレーションを整理する。`calc_Q_UT_A`の分岐前準備はPR #126でコンテキスト化済み。残る前時刻依存、8760時間ループ、床下第1・第2パス、季節分岐は高リスクとして小さく扱い、同じ入力コンテキストを共有する抽出済みヘルパー群だけを安全にまとめる。
