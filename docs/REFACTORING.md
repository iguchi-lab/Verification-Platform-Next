...
以下は候補であり、着手順ではありません。調査結果をIssueに記録し、同じ目的と検証方法を持ち、独立してcommitできる20〜30境界を1つの作業パッケージとして選び、8〜12境界ずつのPRへ分割します。
...
7. 辞書と長い引数列を、既存契約と建研コードとの対応を保ったまま内部DTOや型付き結果へ段階的に整理する。`section4_2_jjj.py`の複数戻り値20境界はPR #72/#73で型付き結果化済みのため、次は入力側の長い引数列を対象とする。

## 1作業パッケージの進め方

1. 最新の`main`へ同期します。
2. `.github/ISSUE_TEMPLATE/refactoring.yml`からIssueを作成します。
3. 作業パッケージ共通の目的、非対象、保持する挙動、検証方法、20〜30件の境界チェックリスト、PR分割案をIssueに記入します。
...
8. PRの8〜12境界が完成したら必須テストをすべて実行し、境界別commitと検証結果をPRへ記録します。
