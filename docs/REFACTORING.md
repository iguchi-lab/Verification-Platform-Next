...
既存モジュールをこの規則へ移行する前に、import元、再公開される名前、外部利用、テストを棚卸しします。Verification Platform Next刷新後は旧importを公開契約として維持せず、呼び出し側の更新と同じPRで互換shimを削除します。既存shimも責務を共有するものを1つの作業パッケージへまとめ、削除後の旧importは即時エラーとします。
...
低リスク領域の1つのPRは50〜80境界、または追加・削除を合わせて1,500〜2,500行程度を目安とします。どちらかの上限を超える前に、レビュー可能な設計境界で分割します。候補が少ない場合、安全にまとめられない場合、停止条件に該当する場合、文書・CIだけの変更では、下限を満たすために変更を水増ししません。

同じ責務、同じ不変条件、同じ対象テストを持つ低リスク処理は、12〜25境界程度を1 commitへまとめられます。特に、同じ方式内の連続処理、単純な変数展開、マスク生成、shape変換、単位変換、DataFrame出力、同じ入力コンテキストを共有するヘルパー群は、評価順と観測可能な挙動を固定できる場合にまとめます。
...
各commitでは対象テストと変更対象のRuffだけを実行します。全体pytest、Phase 5、`ruff check .`、GitHub ActionsはPR単位で1回実行し、commit単位では繰り返しません。計算エンジン内部テストはGitHub Actionsを正式判定とし、低リスクPRでは同じスイートをローカルで重複実行しません。前時刻依存、床下2パス、複雑な季節分岐などの高リスク数値変更、またはCI失敗の診断時だけローカルでも実行します。
...
- [PR #153](https://github.com/iguchi-lab/Verification-Platform-Next/pull/153): 床下計算を床下14仕様へ整合し、7つの確定仕様、負荷の室対応、床下熱損失控除、面積・熱貫流率の使い分け、非反復2パス計算、Excel異常レビューを文書と回帰試験で固定しました。

この一覧は完了した設計判断を把握するためのものです。次の作業は必ず最新の`main`とGitHub上のIssue・PRを確認して決めます。

## 現在の安定点

2026-07-26の`ver.1.0.0`時点で、通常の挙動維持リファクタリングは一旦停止します。`calc_main`は104行、`calc_Q_UT_A`は72行のフェーズ調整役となり、`pyhees -> jjjexperiment`逆依存は0件、Next刷新前の互換shimは削除済み、計算成果物名は正本化済みです。global更新、製品コードの`chdir`、公開調整役へのインプレース処理再流入、8760時間ループの配置はPR #148の契約テストで固定します。建築研究所公式`pyhees`はPR #150でVer.3.10相当へ更新済み、床下計算はPR #153で床下14仕様へ整合済みです。この安定点以降も通常の構造リファクタリング停止を継続します。
...
- 床下第1・第2パス
- 暖房・冷房・中間期の複雑な分岐
- `ceil`・`floor`・`clip`など数値結果に直結する処理
- 数値配列のインプレース更新
- 建研式番号、変数名、評価順と直接対応する処理
- 計算単位を隔離するUIの作業ディレクトリ境界

これらは、具体的な不具合、機能追加、上流更新、計測済みの性能問題のいずれかが発生し、変更目的と追加回帰テストを専用Issueで定義した場合だけ再開します。「さらにきれいにする」ことだけを理由に変更しません。

## 次の候補

定常的なリファクタリング候補はありません。ユーザーの指示により一旦停止中です。再開時は最新`main`と契約テストを確認し、上記の再開条件に対応する1つの設計成果だけを新しい作業パッケージとしてIssue化します。
...
5. 高リスク境界は小さく保ち、同じ責務・不変条件・対象テストを持つ低リスク境界は12〜25境界程度を1 commitへまとめます。
6. 各commitで対象テストと変更対象のRuffを実行し、成功後もブランチが実行可能であることを保ちます。
7. 境界が停止条件に該当した場合はPRまたは作業パッケージから外し、残りの境界を続行します。
8. 低リスク領域のPRが50〜80境界または1,500〜2,500行程度に達する前に区切り、全体pytest、Phase 5、`ruff check .`をPR単位で1回実行します。計算エンジン内部テストは通常GitHub Actionsだけで実行し、高リスク数値変更またはCI失敗の診断時に限りローカルでも実行します。
...
低リスク領域の1PRは50〜80境界または1,500〜2,500行程度を目安とし、どちらかの上限を超える前に分割してください。同じ責務と不変条件を持つ低リスク境界は12〜25境界程度を1 commitにまとめ、高リスク境界は件数目安より安全性を優先して小さくしてください。
各commitでは対象テストと変更対象のRuffだけを実行し、各PR完成後に全体pytest、Phase 5、ruff check .、GitHub Actionsを1回実行してください。計算エンジン内部テストはGitHub Actionsを正式判定とし、低リスクPRではローカルで重複実行せず、高リスク数値変更またはCI失敗の診断時だけローカルでも実行してください。
...
Issueがまだない場合は、実装を始めず、コードを調査して1つの設計上の成果を作業パッケージIssueとして提案します。低リスク領域のPRは50〜80境界または1,500〜2,500行程度を目安とし、どちらかの上限を超える前に分割します。安全にまとめられない候補は別の作業パッケージへ分離します。
