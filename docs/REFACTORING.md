...
- [PR #66](https://github.com/iguchi-lab/Verification-Platform-Next/pull/66): 過剰熱量繰越なし経路の式(9)/(8)負荷、計算モデル別能力、式(20)/(19)熱源入口、熱源出口要求・床下1st補正、出口・供給状態・床下2nd補正の5境界を分離し、モデル分岐、能力上限、診断順、新旧床下補正を維持したまま`calc_Q_UT_A`を76行縮小しました。
...
4. `calc_Q_UT_A` の次段階として、過剰熱量繰越なし経路の式(46)/(48)実室温、繰越あり時刻ループの式(20)/(19)入口状態、出口要求・供給状態、時刻別式(46)/(48)実室温、繰越有無分岐後の共通出力準備を、式番号、時刻スライス、評価順、出力列順を維持した5境界へ分離する。
