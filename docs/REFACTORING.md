...
1つのPRは20〜30境界、または追加・削除を合わせて500〜1,000行程度を目安とします。どちらかの上限を超える前に、レビュー可能な設計境界で分割します。候補が少ない場合、安全にまとめられない場合、停止条件に該当する場合、文書・CIだけの変更では、下限を満たすために変更を水増ししません。

同じ責務、同じ不変条件、同じ対象テストを持つ低リスク処理は、3〜5境界程度を1 commitへまとめられます。計算式、評価順、DI登録、CSV・ファイル副作用、公開API、例外、数値結果に触れる高リスク境界は、原則として独立commitにします。各commitでは対象テストと変更対象のRuffだけを実行し、全体pytest、Phase 5、計算エンジン内部テスト、`ruff check .`、GitHub ActionsはPR単位で実行します。
...
- 20〜30境界または1,000行程度を超え、残りの差分と一緒にレビューしにくい
...
以下は候補であり、着手順ではありません。調査結果をIssueに記録し、1つの設計上の成果を作業パッケージとして選びます。PRは20〜30境界または500〜1,000行程度を目安とし、どちらかの上限を超える前にレビュー可能な設計境界で分割します。
...
3. `jjjexperiment`に残るワイルドカードimportを棚卸しし、同じ依存元と検証方法を持つ20〜30境界を目安に明示的なimportへ置き換える。
4. `calc_Q_UT_A` の次段階として、残る8760時刻ループのオーケストレーションと抽出済みヘルパーの長い引数列を、建研の式番号、時刻スライス、評価順を追跡できる内部コンテキストへ段階的に整理する。
5. 既存の`pyhees -> jjjexperiment`逆依存を独立して検証できる境界へ分け、同じ依存方向と検証方法を持つ20〜30境界を目安に削減する。
...
5. 高リスク境界は独立commitにし、同じ責務・不変条件・対象テストを持つ低リスク境界は3〜5境界程度を1 commitへまとめます。
6. 各commitで対象テストと変更対象のRuffを実行し、成功後もブランチが実行可能であることを保ちます。
7. 境界が停止条件に該当した場合はPRまたは作業パッケージから外し、残りの境界を続行します。
8. PRが20〜30境界または500〜1,000行程度に達する前に区切り、全体pytest、Phase 5、計算エンジン内部テスト、`ruff check .`を実行します。
...
1PRは20〜30境界または500〜1,000行程度を目安とし、どちらかの上限を超える前に分割してください。高リスク境界は独立commit、同じ責務と不変条件を持つ低リスク境界は3〜5境界程度を1 commitにまとめてください。
各commitでは対象テストと変更対象のRuff、各PR完成後は全体pytest、Phase 5、計算エンジン内部テスト、ruff check .、GitHub Actionsを実行してください。
...
Issueがまだない場合は、実装を始めず、コードを調査して1つの設計上の成果を作業パッケージIssueとして提案します。PRは20〜30境界または500〜1,000行程度を目安とし、どちらかの上限を超える前に分割します。安全にまとめられない候補は別の作業パッケージへ分離します。
