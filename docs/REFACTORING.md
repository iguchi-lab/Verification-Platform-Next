...
1. カレントディレクトリ、暗黙のCSVファイル名、出力先の組み立てなど、ファイル出力に関する残存副作用を境界へ集約する。type4電中研モデルのDataFrame構築とCSV書き出しはPR #124、`calc_Q_UT_A`の共通後処理はPR #144で境界化済み。公開されているファイル名、列順、encoding、出力順、例外伝播を維持し、パス決定と書き込み実行を分離する。
2. グローバル状態とインプレース更新を最終監査し、安全に隔離できる境界だけを実行コンテキストまたは専用更新関数へ集約する。前時刻依存の8760時間ループ、床下2パス、複雑な季節分岐、数値結果に直結する更新は再設計せず、変更禁止の不変条件と対象テストを文書化して凍結する。既存Ruff違反も変更対象と由来を分類し、挙動維持PRへ無関係な修正を混在させない。
