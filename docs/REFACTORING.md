...
1. グローバル状態、インプレース更新、実行作業ディレクトリを最終監査し、安全に隔離できる境界だけを実行コンテキストまたは専用更新関数へ集約する。成果物名はPR #146で正本化済み。前時刻依存の8760時間ループ、床下2パス、複雑な季節分岐、数値結果に直結する更新、計算単位を隔離するUIの`chdir`は再設計を前提とせず、変更禁止の不変条件と対象テストを文書化して凍結する。既存Ruff違反も変更対象と由来を分類し、挙動維持PRへ無関係な修正を混在させない。
