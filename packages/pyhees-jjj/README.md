...
│   └── pyhees            # 建築研究所のpyhees由来コード。新しい独自変更は直接追加しません。
├── README.md             # このファイルです。
...
`src/pyhees` は建築研究所由来コードとして扱い、新しい独自変更は直接追加しません。変更が必要な場合は、原則として `src/jjjexperiment` 内のラッパーまたはアダプターに実装します。上流更新の取り込みや、やむを得ず `src/pyhees` を変更する場合は、独自機能のPRと分け、変更理由と数値回帰結果を明記してください。基準と更新手順は [`UPSTREAM.md`](UPSTREAM.md) を参照してください。
