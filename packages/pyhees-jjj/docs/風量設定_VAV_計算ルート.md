...
本書の「設備最低風量へ補正する」経路は、入力欄の
「② 全般換気なし時の給気風量下限を補正する」をONにした場合である。
既定のOFFでは、建研本家と同じく区画別全般換気量を式 (43)・式 (44)の下限へ渡す。
...
flowchart TD
    ...
    B -->|なし| B2{"② 風量下限補正を使う?"}
    B2 -->|OFF| D0["建研互換: 区画別換気下限を維持"]
    B2 -->|ON| D["補正: 熱源機側の区画別換気下限 = 0"]
    C --> E{"最低風量を直接入力?"}
    D0 --> E
    D --> E
...
| 任意 | なし・②OFF | 建研式の区画別換気下限を維持 | 建研本家と同じ | 建研本家と同じ |
| なし | なし・②ON | VAV前風量 | 設備最低風量 | 0 m³/h |
| あり | なし・②ON | 式 (43) 後に設備下限を保証 | 設備最低風量 | 0 m³/h |
