...
def test_write_artifact_manifest_records_release_and_input(tmp_path, monkeypatch):
    ...
    path = tmp_path / "case_v1.0.1_manifest.json"
    ...
    assert manifest == {
        ...
        "version": "1.0.1",
        "display_version": "ver.1.0.1",
        "release_date": "2026-08-05",
        "artifact_version": "_v1.0.1",
...
def test_calc_writes_manifest_only_after_success(tmp_path, monkeypatch):
    ...
    assert json.loads(
        (tmp_path / "release-case_v1.0.1_input.json").read_text(encoding="utf-8")
    ) == input_data
    assert events == [
        ...
        (
            "manifest",
            Path("release-case_v1.0.1_manifest.json"),
            input_data,
        ),
    ]
