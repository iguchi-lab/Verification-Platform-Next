...
def _calc_heating_theoretical_efficiencies(
        type, V_fan_mid_H, q_hs_mid_H, q_hs_rtd_C, V_fan_rtd_H, q_hs_rtd_H,
        Theta_ex_d_t, Theta_hs_in_d_t, Theta_hs_out_d_t, V_hs_supply_d_t):
    # (20) 中間暖房能力運転時
    ...
    # (17) 日付dの時刻tにおける暖房時
    e_th_H_d_t = dc_a.calc_e_th_H_d_t(
        type, Theta_ex_d_t, Theta_hs_in_d_t, Theta_hs_out_d_t,
        V_hs_supply_d_t, q_hs_rtd_C)
    return e_th_mid_H, e_th_rtd_H, e_th_H_d_t


def _calc_heating_efficiency_ratio(
        type, q_hs_H_d_t, q_hs_rtd_H, q_hs_min_H, q_hs_mid_H,
        P_hs_mid_H, P_fan_mid_H, P_hs_rtd_H, P_fan_rtd_H, equipment_spec,
        e_th_mid_H, e_th_rtd_H):
    if type == 計算モデル.RAC活用型全館空調_潜熱評価モデル:  #コンプレッサ効率特性
        return jjj_latent.get_e_r_H_d_t(q_hs_H_d_t)
    ...
    # (13) 中間暖房能力運転時
    e_r_mid_H = dc_a.get_e_r_mid_H(
        e_r_rtd_H, e_th_mid_H, q_hs_mid_H, P_hs_mid_H, P_fan_mid_H, equipment_spec)
    # (9) 日付dの時刻tにおける暖房時
    return dc_a.get_e_r_H_d_t(
        q_hs_H_d_t, q_hs_rtd_H, q_hs_min_H, q_hs_mid_H,
        e_r_mid_H, e_r_min_H, e_r_rtd_H)


def _calc_heating_compressor_electricity(q_hs_H_d_t, e_th_H_d_t, e_r_H_d_t):
    # (5) 圧縮機の分、(7) 熱源機の効率
    return dc_a.get_E_E_comp_H_d_t(
        q_hs_H_d_t,
        e_hs_H_d_t=dc_a.get_e_hs_H_d_t(e_th_H_d_t, e_r_H_d_t))


def _sum_heating_electricity(E_E_comp_H_d_t, E_E_fan_H_d_t):
    return E_E_comp_H_d_t + E_E_fan_H_d_t  # (1)


def _calc_type2_heating_rac_electricity(
        region, q_rtd_C, q_rtd_H, e_rtd_H, dualcompressor_H, q_hs_H_d_t,
        q_max_C, q_max_H, input_C_af_H, climateFile):
    return pyhees.section4_3.calc_E_E_H_d_t_2024(
        region, q_rtd_C, q_rtd_H,  # q[W]
...
def _calc_cooling_load(
        Theta_hs_out_d_t, Theta_hs_in_d_t, X_hs_out_d_t, X_hs_in_d_t,
        V_hs_supply_d_t, region):
    # (4) 潜熱/顕熱を使用せずに全熱負荷を再計算する
    q_hs_CS_d_t, q_hs_CL_d_t = dc_a.get_q_hs_C_d_t(
        Theta_hs_out_d_t, Theta_hs_in_d_t, X_hs_out_d_t, X_hs_in_d_t,
        V_hs_supply_d_t, region)
    return q_hs_CS_d_t, q_hs_CL_d_t, q_hs_CS_d_t + q_hs_CL_d_t


def _calc_cooling_theoretical_efficiencies(
        type, V_fan_mid_C, q_hs_mid_C, q_hs_rtd_C, V_fan_rtd_C,
        Theta_ex_d_t, Theta_hs_in_d_t, X_hs_in_d_t, Theta_hs_out_d_t,
        V_hs_supply_d_t):
    # (22) 中間冷房能力運転時
    ...
    # (18) 日付dの時刻tにおける暖房時
    e_th_C_d_t = dc_a.calc_e_th_C_d_t(
        type, Theta_ex_d_t, Theta_hs_in_d_t, X_hs_in_d_t, Theta_hs_out_d_t,
        V_hs_supply_d_t, q_hs_rtd_C)
    return e_th_mid_C, e_th_rtd_C, e_th_C_d_t


def _calc_cooling_efficiency_ratio(
        type, q_hs_C_d_t, q_hs_rtd_C, q_hs_min_C, q_hs_mid_C,
        P_hs_mid_C, P_fan_mid_C, P_hs_rtd_C, P_fan_rtd_C, equipment_spec,
        e_th_mid_C, e_th_rtd_C):
    if type == 計算モデル.ダクト式セントラル空調機:
        ...
        # (13) 定格冷房能力運転時
        e_r_mid_C = dc_a.get_e_r_mid_C(
            e_r_rtd_C, e_th_mid_C, q_hs_mid_C, P_hs_mid_C, P_fan_mid_C, equipment_spec)
        # (9) 日付dの時刻tにおける冷房時
        return dc_a.get_e_r_C_d_t(
            q_hs_C_d_t, q_hs_rtd_C, q_hs_min_C, q_hs_mid_C,
            e_r_mid_C, e_r_min_C, e_r_rtd_C)
    if type == 計算モデル.RAC活用型全館空調_潜熱評価モデル:  #コンプレッサ効率特性
        # TODO: 潜熱評価モデルが 潜熱(q_hs_CL) ではなく 全熱(q_hs_C) を使用してOKか確認
        return jjj_latent.get_e_r_C_d_t(q_hs_C_d_t)
    raise Exception('冷房設備機器の種類の入力が不正です。')


def _calc_cooling_compressor_electricity(q_hs_C_d_t, e_th_C_d_t, e_r_C_d_t):
    # (6) 圧縮機の分、(8) 熱源機の効率
...
def _sum_cooling_electricity(E_E_comp_C_d_t, E_E_fan_C_d_t):
    return E_E_comp_C_d_t + E_E_fan_C_d_t  # (2)


def _calc_type2_cooling_rac_electricity(
        region, q_rtd_C, e_rtd_C, dualcompressor_C, q_hs_CS_d_t,
        q_hs_CL_d_t, q_max_C, input_C_af_C, climateFile):
    return pyhees.section4_3.calc_E_E_C_d_t_2024(
        region, q_rtd_C,  # q[W]
...
def calc_E_E_H_d_t_type1_and_type3(
    ...
    e_th_mid_H, e_th_rtd_H, e_th_H_d_t = _calc_heating_theoretical_efficiencies(
        type, V_fan_mid_H, q_hs_mid_H, q_hs_rtd_C, V_fan_rtd_H, q_hs_rtd_H,
        Theta_ex_d_t, Theta_hs_in_d_t, Theta_hs_out_d_t, V_hs_supply_d_t)

    """ e_r: ヒートポンプサイクルの理論効率に対する熱源機の効率の比(-) """
    e_r_H_d_t = _calc_heating_efficiency_ratio(
        type, q_hs_H_d_t, q_hs_rtd_H, q_hs_min_H, q_hs_mid_H,
        P_hs_mid_H, P_fan_mid_H, P_hs_rtd_H, P_fan_rtd_H, equipment_spec,
        e_th_mid_H, e_th_rtd_H)

    """ E_E: 日付dの時刻tにおける1時間当たりの暖房時の消費電力量 (kWh/h) """
    E_E_comp_H_d_t = _calc_heating_compressor_electricity(
        q_hs_H_d_t, e_th_H_d_t, e_r_H_d_t)
    return _sum_heating_electricity(E_E_comp_H_d_t, E_E_fan_H_d_t)
...
def calc_E_E_H_d_t_type2(
    ...
    E_E_CRAC_H_d_t = _calc_type2_heating_rac_electricity(
        region, q_rtd_C, q_rtd_H, e_rtd_H, dualcompressor_H, q_hs_H_d_t,
        q_max_C, q_max_H, input_C_af_H, climateFile)
    return _sum_heating_electricity(E_E_fan_H_d_t, E_E_CRAC_H_d_t)
...
def calc_E_E_C_d_t_type1_and_type3(
    ...
    q_hs_CS_d_t, q_hs_CL_d_t, q_hs_C_d_t = _calc_cooling_load(
        Theta_hs_out_d_t, Theta_hs_in_d_t, X_hs_out_d_t, X_hs_in_d_t,
        V_hs_supply_d_t, region)

    """ e_th: ヒートポンプサイクルの理論効率(-) """
    e_th_mid_C, e_th_rtd_C, e_th_C_d_t = _calc_cooling_theoretical_efficiencies(
        type, V_fan_mid_C, q_hs_mid_C, q_hs_rtd_C, V_fan_rtd_C,
        Theta_ex_d_t, Theta_hs_in_d_t, X_hs_in_d_t, Theta_hs_out_d_t,
        V_hs_supply_d_t)

    """ e_r: ヒートポンプサイクルの理論効率に対する熱源機の効率の比(-) """
    e_r_C_d_t = _calc_cooling_efficiency_ratio(
        type, q_hs_C_d_t, q_hs_rtd_C, q_hs_min_C, q_hs_mid_C,
        P_hs_mid_C, P_fan_mid_C, P_hs_rtd_C, P_fan_rtd_C, equipment_spec,
        e_th_mid_C, e_th_rtd_C)

    """ E_E: 日付dの時刻tにおける1時間当たりの冷房時の消費電力量 (kWh/h) """
    E_E_comp_C_d_t = _calc_cooling_compressor_electricity(
        q_hs_C_d_t, e_th_C_d_t, e_r_C_d_t)
    return _sum_cooling_electricity(E_E_comp_C_d_t, E_E_fan_C_d_t)
...
def calc_E_E_C_d_t_type2(
    ...
    """ 顕熱/潜熱 (CS/CL) を使用する """
    # NOTE: 別モジュールから同名の関数を利用しています
    E_E_CRAC_C_d_t = _calc_type2_cooling_rac_electricity(
        region, q_rtd_C, e_rtd_C, dualcompressor_C, q_hs_CS_d_t,
        q_hs_CL_d_t, q_max_C, input_C_af_C, climateFile)

    _logger.NDdebug("E_E_CRAC_C_d_t", E_E_CRAC_C_d_t)
    return _sum_cooling_electricity(E_E_CRAC_C_d_t, E_E_fan_C_d_t)
