...
class TestVMinInput送風機消費電力:
    ...
    def test_直線近似法(self):
        ...
        result = get_E_E_fan_d_t(
            ファン消費電力算定方法.直線近似法,
            self.P_fan_rtd, self.V_hs_vent, self.V_hs_supply, self.V_hs_dsgn, self.q_hs,
            self.E_E_fan_min
        )
        assert result[0] == pytest.approx(0.068)
        assert result[1] == 0.0

    def test_風量三乗近似法(self):
        # y = ax^3 + b
        # a = (y2 - y1) / (x2^3 - x1^3) = (100 - 20) / (300^3 - 50^3)
        # a = 80 / (27,000,000 - 125,000) = 80 / 26,875,000 = 8 / 2,687,500 = 1 / 335,937.5
        ...
        assert result[0] == pytest.approx(expected_result)

    def test_サーモオフ時の風量(self):
        # q_hs <= 0 の場合、結果は 0 になる（フィルタリングされるため）
        self.q_hs[0] = 0.0
        result = get_E_E_fan_d_t(
            ファン消費電力算定方法.直線近似法,
            self.P_fan_rtd, self.V_hs_vent, self.V_hs_supply, self.V_hs_dsgn, self.q_hs,
...
class TestVMinInput送風機消費電力_冷房:
    ...
    def test_直線近似法_冷房(self):
        ...
        assert result[100] == pytest.approx(expected_result)
